...
class Chrmrtns_KLA_2FA_Frontend {
    ...
    public function render_2fa_shortcode($atts = array()) {
        ...
        if (!get_option('chrmrtns_kla_2fa_enabled', false)) {
            return '<div class="chrmrtns-2fa-notice">' .
            ...
            <div class="chrmrtns-2fa-step">
                ...
                <div class="chrmrtns-2fa-qr-section">
                    <div class="chrmrtns-2fa-qr">
                        <div id="chrmrtns-2fa-qrcode" data-totp-uri="<?php echo esc_attr($totp_uri); ?>">
                            <div class="chrmrtns-qr-loading"><?php esc_html_e('Loading QR code...', 'keyless-auth'); ?></div>
                        </div>

                        <script>
                        // Inline QR library to bypass caching issues
                        <?php
                        $qr_js_file = CHRMRTNS_KLA_PLUGIN_DIR . 'assets/js/qrcode.min.js';
                        if (file_exists($qr_js_file)) {
                            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- JavaScript library content, intentionally unescaped
                            echo file_get_contents($qr_js_file);
                        }
                        ?>

                        // Inline 2FA frontend JavaScript to bypass server file serving issues
                        console.log('2FA frontend script loaded via inline');

                        jQuery(document).ready(function($) {
                            console.log('jQuery ready, QRCode available:', typeof QRCode !== 'undefined');
                            ...
                            if (qrContainer.length) {
                                ...
                                if (totpUri) {
                                    ...
                                    console.log('No TOTP URI data found');
                    ...
                    <div class="chrmrtns-2fa-manual">
                        ...
                        <div class="chrmrtns-2fa-secret">
                            ...
                            <button type="button" class="chrmrtns-copy-button" data-copy="<?php echo esc_attr($secret); ?>">
    ...
    private function render_2fa_active_view($user, $settings, $role_required) {
        ...
        <div class="chrmrtns-2fa-header">
            <h3><?php esc_html_e('Two-Factor Authentication', 'keyless-auth'); ?></h3>
            <div class="chrmrtns-2fa-status chrmrtns-2fa-enabled">
                <span class="chrmrtns-2fa-status-icon">✓</span>
