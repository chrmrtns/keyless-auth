...
class Chrmrtns_Mail_Logger {
    ...
    public function handle_mail_logs_actions() {
        ...
        if (isset($_POST['chrmrtns_mail_logging_submit'])) {
            if (!wp_verify_nonce($_POST['chrmrtns_mail_logs_nonce'], 'chrmrtns_mail_logs_settings')) {
                add_settings_error(
            ...
            add_settings_error(
                ...
                __('Mail logging settings saved successfully!', 'chrmrtns-passwordless-auth'),
                'updated'
            );
        }
        ...
        if (isset($_POST['chrmrtns_clear_mail_logs'])) {
            if (!wp_verify_nonce($_POST['chrmrtns_clear_logs_nonce'], 'chrmrtns_clear_mail_logs')) {
                add_settings_error(
                    'chrmrtns_mail_logs_settings',
            ...
            add_settings_error(
                ...
                sprintf(__('Successfully deleted %d mail logs.', 'chrmrtns-passwordless-auth'), count($logs)),
                'updated'
            );
        }
        ...
        if (isset($_POST['chrmrtns_delete_log']) && isset($_POST['chrmrtns_delete_log_id'])) {
            if (!wp_verify_nonce($_POST['chrmrtns_delete_log_nonce'], 'chrmrtns_delete_mail_log')) {
                add_settings_error(
                    'chrmrtns_mail_logs_settings',
            ...
            if ($log_id > 0) {
                ...
                add_settings_error(
                    ...
                    __('Mail log deleted successfully.', 'chrmrtns-passwordless-auth'),
                    'updated'
                );
            }
        }
    ...
    public function log_mail_event($mail_data) {
        ...
        $post_data = array(
            'post_type'   => 'chrmrtns_mail_logs',
            'post_status' => 'publish',
            'post_title'  => __('Mail Log', 'chrmrtns-passwordless-auth') . ' - ' . date('Y-m-d H:i:s')
        );
    ...
    public function render_mail_logs_page() {
        ...
        <div class="wrap chrmrtns-wrap">
            <h1><?php _e('Mail Logs', 'chrmrtns-passwordless-auth'); ?></h1>
            <p><?php _e('Track and monitor all emails sent from your WordPress site. Enable logging to see detailed information about sent emails including recipients, subjects, and content.', 'chrmrtns-passwordless-auth'); ?></p>
            ...
            <form method="post" action="">
                ...
                <table class="form-table">
                    <tr>
                        <th scope="row"><?php _e('Enable Mail Logging', 'chrmrtns-passwordless-auth'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="chrmrtns_mail_logging_enabled" <?php checked($logging_enabled, true); ?> value="1">
                                <?php _e('Enable mail logging for all emails sent from WordPress', 'chrmrtns-passwordless-auth'); ?>
                            </label>
                            <p class="description"><?php _e('When enabled, all emails sent via wp_mail() will be logged and stored.', 'chrmrtns-passwordless-auth'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><?php _e('Log Storage Limit', 'chrmrtns-passwordless-auth'); ?></th>
                        <td>
                            ...
                            <p class="description"><?php _e('Maximum number of mail logs to keep. Older logs will be automatically deleted when this limit is reached.', 'chrmrtns-passwordless-auth'); ?></p>
                        </td>
                    </tr>
                </table>
                <?php submit_button(__('Save Settings', 'chrmrtns-passwordless-auth'), 'primary', 'chrmrtns_mail_logging_submit'); ?>
            </form>
            ...
            <?php if ($logging_enabled): ?>
                ...
                <div class="tablenav top">
                    <form method="post" action="" style="float: left;" onsubmit="return confirm('<?php esc_attr_e('Are you sure you want to delete all mail logs? This action cannot be undone.', 'chrmrtns-passwordless-auth'); ?>');">
                        <?php wp_nonce_field('chrmrtns_clear_mail_logs', 'chrmrtns_clear_logs_nonce'); ?>
                        <?php submit_button(__('Clear All Logs', 'chrmrtns-passwordless-auth'), 'delete', 'chrmrtns_clear_mail_logs', false); ?>
                    </form>
                    <div class="clear"></div>
                </div>
                ...
                if (empty($logs)): ?>
                    <p><?php _e('No mail logs found. Emails will appear here once they are sent.', 'chrmrtns-passwordless-auth'); ?></p>
                <?php else: ?>
                    <table class="wp-list-table widefat fixed striped">
                        <thead>
                            <tr>
                                <th><?php _e('Date & Time', 'chrmrtns-passwordless-auth'); ?></th>
                                <th><?php _e('From', 'chrmrtns-passwordless-auth'); ?></th>
                                <th><?php _e('To', 'chrmrtns-passwordless-auth'); ?></th>
                                <th><?php _e('Subject', 'chrmrtns-passwordless-auth'); ?></th>
                                <th><?php _e('Actions', 'chrmrtns-passwordless-auth'); ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($logs as $log): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="button button-small" onclick="chrmrtnsShowEmailContent(<?php echo $log->ID; ?>)"><?php _e('View Content', 'chrmrtns-passwordless-auth'); ?></button>
                                        <form method="post" style="display: inline;" onsubmit="return confirm('<?php esc_attr_e('Are you sure you want to delete this log?', 'chrmrtns-passwordless-auth'); ?>');">
                                            <?php wp_nonce_field('chrmrtns_delete_mail_log', 'chrmrtns_delete_log_nonce'); ?>
                                            <input type="hidden" name="chrmrtns_delete_log_id" value="<?php echo $log->ID; ?>">
                                            <?php submit_button(__('Delete', 'chrmrtns-passwordless-auth'), 'delete button-small', 'chrmrtns_delete_log', false); ?>
                                        </form>
                                        
                                        <div id="chrmrtns_email_content_<?php echo $log->ID; ?>" style="display: none; margin-top: 10px; padding: 10px; border: 1px solid #ddd; background: #f9f9f9;">
                                            <h4><?php _e('Email Content:', 'chrmrtns-passwordless-auth'); ?></h4>
                                            <div style="max-height: 300px; overflow-y: auto; background: white; padding: 10px; border: 1px solid #ccc;">
                                                <?php echo wp_kses_post($message); ?>
                                            </div>
                                            <button type="button" class="button button-small" onclick="chrmrtnsHideEmailContent(<?php echo $log->ID; ?>)" style="margin-top: 10px;"><?php _e('Hide Content', 'chrmrtns-passwordless-auth'); ?></button>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                ...
                <?php endif; ?>
            <?php else: ?>
                <p><em><?php _e('Mail logging is currently disabled. Enable it above to start tracking emails.', 'chrmrtns-passwordless-auth'); ?></em></p>
            <?php endif; ?>
        </div>
